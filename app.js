...
const schedules = {
  ...
  return: {
    title: "道远楼东停车场 → 泰瑞府",
    detail: "从道远楼东停车场发车（经停张灵斌楼/综合教学楼）",
...
const returnStops = {
  daoyuan: { label: "道远楼东停车场", offset: 0 },
...
function render() {
  ...
  if (routeDetailElement) {
    routeDetailElement.textContent = selectedDirection === "return"
      ? `从道远楼东停车场发车 · ${returnStop.label}上车`
      : route.detail;
  }
  ...
  heroCard.innerHTML = `
    ...
    <div class="departure-row">
      <div>
        ...
        ${selectedDirection === "outbound" ? `<p class="origin-departure">预计到校：${addMinutes(nextBus.time, 10)}</p>` : (nextBus.secondaryTime ? `<p class="origin-departure">道远楼东停车场始发：${nextBus.secondaryTime}</p>` : "")}
      </div>
    </div>
    ${lastBusBannerHtml}
...
function renderQuery() {
  ...
  rows.forEach(({ primary, source }) => {
    ...
    cell.addEventListener("click", () => {
      ...
      if (source) toastMsg += `（道远楼东停车场发车 ${source}）`;
      if (isTodaySchedule) {
...
var SHUTTLE_NOTICE_SUBTITLES = {"2026-09-17":"Test at 9:30 9/17"};
(function () {
  try {
