...
// 远程临时排班：由排班后台通过 GitHub API 写入仓库根目录的 overrides.json，
// 按日期映射保存；仅"日期等于今天"的条目生效，其他日期与过期数据自动忽略。
let remoteScheduleOverrideMap = {};
const scheduleOverrideRemotePath = "overrides.json";
const scheduleOverrideRemoteInterval = 5 * 60 * 1000;

const SCHEDULE_OVERRIDE_DATE_PATTERN = /^\d{4}-\d{2}-\d{2}$/;

// 把远程数据规范化为 { "YYYY-MM-DD": entry }，兼容多日期格式与旧版单日格式
function normalizeRemoteScheduleOverride(data) {
  if (!data || typeof data !== "object" || Array.isArray(data)) return {};
  let source = null;
  if (data.overrides && typeof data.overrides === "object" && !Array.isArray(data.overrides)) {
    source = data.overrides;
  } else if (typeof data.date === "string" && SCHEDULE_OVERRIDE_DATE_PATTERN.test(data.date)) {
    source = { [data.date]: data };
  }
  if (!source) return {};

  const map = {};
  Object.entries(source).forEach(([key, entry]) => {
    if (!SCHEDULE_OVERRIDE_DATE_PATTERN.test(key) || !entry || typeof entry !== "object") return;
    if (entry.type !== "weekday" && entry.type !== "weekend") return;
    map[key] = {
      type: entry.type,
      notice: entry.notice && typeof entry.notice === "object" && entry.notice.subtitle
        ? { subtitle: String(entry.notice.subtitle) }
        : null
    };
  });
  return map;
...
async function fetchRemoteScheduleOverride() {
  let data = null;
  try {
    const response = await fetch(`${scheduleOverrideRemotePath}?t=${Date.now()}`, { cache: "no-store" });
    if (response.ok) {
      const parsed = await response.json();
      data = parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed : null;
    }
  } catch {
    return; // 拉取失败时静默降级，保持原有判断逻辑
  }
  const previousType = getScheduleType(new Date());
  remoteScheduleOverrideMap = normalizeRemoteScheduleOverride(data);
  updateNoticeBanner();
  if (getScheduleType(new Date()) !== previousType) {
    currentActiveDayType = getScheduleType(new Date());
    selectedQueryScheduleType = currentActiveDayType;
    if (scheduleTypeSelect) scheduleTypeSelect.value = currentActiveDayType;
    syncPillButtons(currentActiveDayType);
    render();
    renderQuery();
    showToast(`今日临时排班已更新：${currentActiveDayType === "weekday" ? "工作日" : "周末"}时刻表`);
  }
}

// 顶部告示横幅：只针对"今天"的调整，未来/过期日期不显示
function updateNoticeBanner() {
  const banner = document.querySelector("#notice-banner");
  if (!banner) return;
  const today = dateKey(new Date());
  const override = remoteScheduleOverrideMap[today];
  if (!override) {
    banner.hidden = true;
    return;
  }
  banner.hidden = false;
  const titleElement = document.querySelector("#notice-banner-title");
  const subtitleElement = document.querySelector("#notice-banner-subtitle");
  if (titleElement) {
    titleElement.textContent = `今日调整为${override.type === "weekday" ? "工作日" : "周末"}时间表`;
  }
  const subtitle = override.notice && override.notice.subtitle;
  if (subtitleElement) {
...
function getScheduleTypeInfo(date) {
  ...
  // 排班后台写入的远程数据优先级最高，仅匹配"这一天"自己的日期，互不影响。
  const remoteEntry = remoteScheduleOverrideMap[key];
  if (remoteEntry) {
    return {
      type: remoteEntry.type,
      desc: remoteEntry.type === "weekday" ? "工作日服务时刻表" : "周末及法定节假日时刻表"
    };
  }
...
// 定期重新拉取远程排班数据，让已打开的页面也能自动跟随后台调整
setInterval(fetchRemoteScheduleOverride, scheduleOverrideRemoteInterval);
